...
from helpers import expDecay, indPrint
...
def mk_per(start, end, con):
  ...
  if con.debugNests>=5:
    # print( ">> start & end of nest period:\n", nestPeriod, nestPeriod.shape)
    # print( f">> start & end of nest period (length:{nestPeriod.shape}):\n")
    print( f">> start & end of nest period:\n")
    pprint.pprint(nestPeriod, indent=4, compact=True, width=90)
  return(nestPeriod)
...
def assign_fate(assignVal, pWrong, trueFate, numNests, obsFr, intFinal, stormFate, cn):
  ...
  if cn.debugObs>=3:
    ...
    print(trueFate, len(trueFate))
    print("\t>-> assigned fates before (all nests, not just discovered):")
    print(assignedFate, len(assignedFate))
  if stormFate: assignedFate[intFinal > obsFr] = 2
  if cn.debugObs>=2:
    print(
        f"\t\t>-> true fate counts:\tH:{sum(trueFate==0)}|"
        f"D:{sum(trueFate==1)}|F:{sum(trueFate==2)}"
        )
...
def svy_position(initiation, nestEnd, surveyDays, cn):
  ...
  if cn.debugObs>=5: print(">>>> position of initiation date in survey day list:\n", position, len(position)) 
  if cn.debugObs>=5: print(">> end dates:\n", nestEnd)
  position2 = np.searchsorted(surveyDays, nestEnd)
  if cn.debugObs>=5: print(">>>> position of end date in survey day list:\n", position2, len(position2)) 
  surveyDays = dict(zip(np.arange(len(surveyDays)), surveyDays))
  if cn.debugObs>=5: print(">> survey days with index number:\n", surveyDays)
...
def observer(nData, par, surveys, out, conf):
  ...
  if conf.debugObs>=2:
    ...
    print(
        f"\t\t>-> assigned fate counts (discovered):\tH:{sum(assignedFate==0)}|"
        f"D:{sum(assignedFate==1)}|F:{sum(assignedFate==2)}"
        )
    print("\t>-> true fates (discovered nests):",
          trueFate[discovered], len(trueFate[discovered]))
    print("\t>-> assigned fates (discovered nests):",
          # assignedFate[discovered], len(assignedFate[discovered]))
  ...
  if conf.debugObs>=1: 
    ...
    print(
      ...
      "vs period survival:",
      round(par.probSurv**par.hatchTime,5)
      )
  if conf.debugObs==2: 
