...
#---- Make params, storms, surveys: --------------------------------------------------------------
  ...
  # print(par)
  # print(class(par))
  # cat("\n\n.....",format(startTimePar, "%H:%M:%S"),"......i=",i, ".......seed=",rngSeed,"........................................................................................................\n")
...
#---- LOOP THRU REPLICATES ------------------------------------------------------------------------------
  ...
  #---- Logistic exposure: -----------------------------------------------------------------------------------------------
    ...
    if(config$logex){
      ...
      #   next
      # }
       ## try survey::svyvar() to get weighted variance
      # mList <- mList[c(1:3)]
      # cat("\n\tmList=",  mList)
    ...
    if(config$logex){
      ...
      # prList <- marginaleffects::predictions(modFit[[2]])
      # cat("\n\t|> model 2 predictions from marginaleffects:\n")
      # print(prList)

      # numInit     <- sapply(newDat$Date, function(x) sum(nestData$init==x))
      # numInit     <- sapply(dat2S$Date, function(x) sum(nestData$init==x))
      # propInit    <- numInit/par$numNests
      # propInitScl <- propInit/sum(propInit) ## make sure it sums to 1
      # cat("\nlength of numInit & propInitScl:",length(numInit),length(propInitScl))
      # X <- model.matrix(modFit[[2]])
      # beta <- coef(modFit[[2]])
      # vc <- vcov(modFit[[2]])
      # predictor <- X %*% beta
      # prd <- 1 / (1+exp(-predictor)) 
      # link_se <- sqrt(diag(X %*% vc %*% t(X)))
      # cat("\n\t|> model 2 standard error on link scale, length", length(link_se))
      # print(t(link_se))
      # prd_se <- 1 / (1+exp(-link_se)) 
      # cat("\n\t|> weighted predictions & se, link scale:")
      # wt_prd <- make_weighted(predictor,link_se,nestData$init, dat2S$Date,
      #               par, config, type="link")
      # print(wt_prd)
      

      # prd_se <- msm::deltamethod()

      # cat("\n\t|> model 2 predictions from matrix multiplication (&se) :\n")
      # print(t(prd))
      # print(t(prd_se))
      # coefs <- get_coef(modFit)
      # DSRlist2 <- make_dsr_list(dat2S,survey,mList,par,pyconfig,out=modFit)
      # cat("\n\t>> calling make_dsr_list")
      # DSRout <- make_dsr_list(dat2S,survey,mList,par,pyconfig)
      DSRout <- make_dsr_list(dat2S,survey,mList,par,pyconfig, out=modFit)
      ...
      # predList <- lapply(modFit, function(x) predict(x, newdata=dat2S,type="response",se.fit=T))
      # rm(..exposure)
      # print(DSRlist)
      # print(DSRlist[-1])
      ...
      # newDat <- expand.grid(Date=prDays, Age=seq(1,par$hatchTime), avDate=80)
      # print(newDat)
      # if(config$coefSave=="yes"){
      #   # dateDat    <- dat2S
      #   # numInit     <- sapply(dat2S$Date, function(x) sum(nestData$init==x))
      ...
      #   propInitScl <- propInit/sum(propInit) ## make sure it sums to 1
      #
      #   DSRval_mat <- do.call(rbind, DSRlist)
      #   modDSRmat[c(1:5),,r,i] <- DSRval_mat
      #   modDSRmat[6,,r,i] <- propInitScl
      #
      #   if(config$testing=="yes"){
      #     cat("\n\t> numInit & propInitScl:")
      #     qvcalc::indentPrint(numInit)
      #     qvcalc::indentPrint(propInitScl)
      #     print(dim(DSRval_mat))
      #     print(dim(modDSRmat[c(1:5),,r,i]))
      #   }
      # }
      # dsr1 <- DSRlist[[1]][1]
      dsr1 <- DSRlist[[1]][[1]][1]
      ...
      # out_list <- sapply(DSRlist[-1], function(x){
      # out_list <- sapply(seq_along(DSRlist[-1]), function(x){
      # cat("\t>> making weighted list")
      ...
      # alpha <- 0.05
      # weightDSR <- lapply(DSRlist[-1], function(x) Hmisc::wtd.mean(x, weights=propInitScl, na.rm=FALSE))
      # cat("\nweighted DSR:", class(weightDSR), unlist(weightDSR))
      # weightVar <- lapply(DSRlist[-1], function(x) Hmisc::wtd.var(x, weights=propInitScl, na.rm=FALSE))
      # cat("\nweighted variance:", class(weightVar), unlist(weightVar))
      # weightSE <- sqrt(unlist(weightVar)/n)
      # print(out_list)
      # print(class(out_list))
      # out_list <- make_psr(DSRlist[[2]],nestData$init,dat2S$Date,par,config)
      dsr_list <- out_list[1,]
      psr_list <- out_list[2,]
      se_list <- out_list[3,]

      # cat("\t>> making prediction list")
      # ..exposure <- mean(dat2S$Exposure)
      # # predList <- lapply(modFit, function(x) predict(x, newdata=dat2S,type="response",se.fit=T))
      # predList <- lapply(modFit, function(x) predict(x,type="response",newdata=newDat,se.fit=T))
      # rm(..exposure)
      # cat("\t>> making weighted prediction list")
      # wtpredList <- sapply(seq(2,length(predList)), function(x){
      #                               make_weighted(predList[[x]]$fit,
      #                                             predList[[x]]$se.fit,
      #                                             nestData$init,
      #                                             dat2S$Date,
      #                                             # newDat$Date,
      ...
      #   cat("\n\t\tmaking predList & weighted predList:")
      #   qvcalc::indentPrint(predList)
      #   qvcalc::indentPrint(wtpredList)
      # }
      # print(dim(predList))
      # print(dsr_list)
      # print(class(dsr_list))
      # print(psr_list)
      # print(class(psr_list))
      # logexVal <- unlist(tibble::lst(dsr1,psr1,psr_list))
      # logexVal <- unlist(tibble::lst(dsr1,psr1,dsr_list,psr_list))
  ...
  #---- MCMC model: ------------------------------------------------------------------------------------------------------
    ...
    # if(TRUE){
    if(config$mcmc){
      # skiptoNext <- FALSE ## needs to be one level above?
      ...
      # mlOut <- mlfun$PolyMort(obsInp, config)
      jacPlSuf <- sprintf("%s-%s",i,r)
      # mlOut <- mlfun$PolyMort(obsSel,survey,par, config,suff=jacPlSuf)
      # mlOut <- mlfun$PolyMort_new(obsSel,survey, config,suff=jacPlSuf)
      # mlOut <- mlfun$jPolyMort(obsSel,survey, config,suff=jacPlSuf)
      # cat("\nMCMC output:")
      # print(mlOut)
      # print(class(mlOut))
      withCallingHandlers({
      ...
      error=function(e){
        ...
        reticulate::py_last_error()
        withCallingHandlers({
          mlOut <- mlfun$PolyMort(obsSel,survey,par, config,plt=TRUE,db=2,suff=jacPlSuf)
        },
        error=function(e){
          skiptoNext <<- TRUE # need to use super-assignment
  ...
  #---- Add to DSR matrix: -----------------------------------------------------------------------------------------------
    ...
    if(config$testing=="yes"){
      ...
      if(debug>=2) cat(sprintf("\t>> Mayfield standard error: %s", mayfSE))
      if(debug>=4){
